...
export class AdminReviews implements OnInit {
    ...
    this.http.get<any[]>(`${API_CONFIG.API_BASE}/admin/reviews`).subscribe({
      next: (res) => {
        this.reviews = res.map((r) => ({
          id: String(r.id ?? ''),
          user: String(r.user ?? 'Anonymous User'),
          email: String(r.email ?? ''),
          avatar: String(r.avatar ?? ''),
          quizId: String(r.quizId ?? ''),
          quizTitle: String(r.quizTitle ?? 'Unknown Quiz'),
          content: String(r.content ?? ''),
          rating: Number(r.rating ?? 0),
          createdAt: String(r.createdAt ?? '')
        }));
      },
      error: (err) => console.error('Load admin reviews failed:', err)
    });
  }
  ...
  this.http.get<any[]>(`${API_CONFIG.API_BASE}/quizzes`).subscribe({
    ...
    error: (err) => console.error('Load quizzes failed:', err)
  });
}
  ...
  getAvatar(review: AdminReview): string {
    ...
    return `https://api.dicebear.com/8.x/avataaars/svg?seed=${encodeURIComponent(review.email || review.user || review.id)}`;
  }
  ...
  get quizCards(): { id: string; title: string; count: number; avgRating: string }[] {
    return this.allQuizzes.map((quiz) => {
      ...
      return {
        ...
        title: quiz.title,
        count: items.length,
        avgRating: items.length > 0 ? (totalRating / items.length).toFixed(1) : '0.0'
      };
    });
  }
}
